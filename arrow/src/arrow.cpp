...
void ArrowShape::update(sf::Vector2f start, sf::Vector2f end)
{
    ...
    // sfml setRotation function argument requires units of degrees
    float rotationDegrees = std::atan2(direction.y, direction.x) * 180.0f / 3.14159f;
    

    // arrow shaft properties
    m_shaft.setSize(sf::Vector2f{length - m_headSize, m_thickness});
    m_shaft.setOrigin({0.0f, m_thickness/2.0f});
    m_shaft.setPosition(m_start);

    
    // setRotation function argument must be Angle type. 
    ...
    m_head.setOrigin(sf::Vector2f{0.0f, 0.0f});
    m_head.setPosition(m_end);
    m_head.setRotation(sf::degrees(rotationDegrees));    
